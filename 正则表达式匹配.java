...
public class Solution {
    /*
        1. 如果字符串当前位置的下一个字符不是*，那么：
            当前位置字符匹配：字符串和模式的索引均向后移一位，继续匹配之后的；
                    不匹配：返回false
        2. 如果字符串当前位置下一个字符是*，那么：
            如果当前位置字符不匹配：模式索引后移2位，字符串不变，相当于 x* 这两个字符被忽略掉，
                                因为*可以匹配0次；
            如果当前位置字符匹配，有三种方式继续匹配，采取递归来一一试探：
                （1）模式索引后移2位，字符串不变，相当于 x* 这两个字符被忽略掉；
                    这里解释下为什么这么做，
                    举个栗子，"bbbba"和".*a"，如果到了第4个b的时候没有把".*"给跳过去，那么".*"还会
                    继续匹配下个字符a，那么模式中的a就得不到匹配，返回false。实际上如果".*"匹配了4个
                    b，然后再匹配最后那个a，是true的；
                （2）字符串索引后移1位，模式不变，即使用当前字符继续匹配字符串，因为 * 可以匹配多次；
                （3）字符串索引后移1位，模式后移2位，即 x* 只匹配1次
    */
